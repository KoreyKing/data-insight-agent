...
- **订单状态**：如果有这一列，会自动识别常见写法——「已完成」「交易成功」`paid` 记为已完成，「部分退款」`partially refunded` 记为部分退款，「已退款」`returned` 记为已退款；无法识别的取值（如「已取消」）不参与销售额、订单数、客单价与退款率的计算，报告完成时分析对话里会提示涉及多少行。没有这一列时，所有行都按已完成订单计算。

## 数据与隐私

- 数据都留在你的机器上：报告、数据集与配置存放在 `./data`（一个 SQLite 文件加上传的文件）。
- 配置了远程模型时，数据结构、聚合查询结果与提示文本会发送给**你所选择的模型服务商**；原始单行明细不会发送。
- 体验模式不调用任何模型；网页界面不加载任何第三方资源（字体随应用打包）。

## 安全提示

- 应用**没有登录与用户管理**。任何能访问 8000 端口的人都能查看你的报告、修改模型配置。
- `docker-compose.yml` 默认把 8000 端口只绑定到 `127.0.0.1`，只有本机能打开。需要从其他设备访问时，把端口映射改为 `"8000:8000"`，并只在可信网络中使用；不要在没有带认证的反向代理的情况下把它暴露到公网。
...
启动时会自动升级数据库结构（只新增表与列，保留已有数据）。从未编辑过的业务口径会随升级更新到新的出厂版本；编辑过的保持不变（想采用新的出厂口径，可在「业务口径设置」里恢复默认）。升级前请先备份 `./data` 目录。
...
- **打开 8000 是空白或连不上**：确认 `docker compose up -d` 已成功、容器在运行；首次启动需要时间拉取镜像。从其他设备访问时见[安全提示](#安全提示)：端口默认只监听本机。
